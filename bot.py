    ...
    for symbol in symbols:
        try:
            ...
            if latest_vol >= (2 * vol_sma20):
                ...
                # Pachhli 3 closed candles (iloc[-5], iloc[-4], iloc[-3])
                prev_c1 = df.iloc[-3]
                prev_c2 = df.iloc[-4]
                prev_c3 = df.iloc[-5]
                ...
                msg = (
                    ...
                    f"1️⃣ Prev 1: Price `${prev_c1['close']}` | Vol `{prev_c1['volume']:,.2f}`\n"
                    f"2️⃣ Prev 2: Price `${prev_c2['close']}` | Vol `{prev_c2['volume']:,.2f}`\n"
                    f"3️⃣ Prev 3: Price `${prev_c3['close']}` | Vol `{prev_c3['volume']:,.2f}`"
                )
